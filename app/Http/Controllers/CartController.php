...
class CartController extends Controller
{
    public function add_to_cart(Request $request)
    {
        ...
        [
            'price' => $prodcut->sale_price ? $prodcut->sale_price : $prodcut->price,
    ...
    public function checkout()
    {
        ...
        //this is not an error: coz i didn't call the model of user, so the model dont see this {carts()} أصلا here im calling the Auth Model...
        // dd($total);
    ...
    if($code == '000.100.110') {
        ...
        DB::beginTransaction();
        try {
                //create new order (done)
            $order = Order::create([
                'total' => $amount,
                'user_id' => Auth::id(),
            ]);

            //add carts to order items (done)
            foreach(Auth::user()->carts as $cart){
                ...
                //decrease the product quantity (done)
                $cart->product()->decrement('quantity', $cart->quantity);

                //delete user cart(done)
                $cart->delete();
            }
